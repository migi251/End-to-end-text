...
def benchmark_all_eval(model, criterion, converter, opt, calculate_infer_time=False):
    ...
    for eval_data in eval_data_list:
        ...
        _, accuracy_by_best_model, norm_ED_by_best_model, _, _, infer_time, length_of_data = validation(
            model, criterion, evaluation_loader, converter, opt,eval_data=eval_data)
...
def validation(model, criterion, evaluation_loader, converter, opt,eval_data=None):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        img = image_tensors[100].squeeze().mul_(0.5).add_(0.5).mul_(255).numpy()
        print(img.shape)
        cv2.imshow('1',img)
        cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    opt.eval_data='data_lmdb_release/evaluation'
    ...
    opt.n_layers_enc=6
    opt.n_layers_dec=6
    opt.d_inner_hid=1024
    opt.batch_size=500
    opt.saved_model='/home/migi/Workspace/DATN/ComposeRecognition/saved_models/new-model-450-SimpleConv-enc-6-dec-6-d-inner-1024-n-warmup-16000/iter_249000.pth'
    test(opt)
