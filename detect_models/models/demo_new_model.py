...
class AAA(nn.Module):
    ...
    def _make_upsample(self, expansion=1):
        if self.backbone_name == 'vgg16':
            ...
            self.deconv5 = Upsample(512*expansion, 256, 2)
            self.merge4 = MergeUpsample(
                256+256*expansion, 256)
            self.merge3 = MergeUpsample(
                256+ 128*expansion, 256)
            self.merge2 = MergeUpsample(
                256 + 64*expansion, 256)
            self.merge1 = MergeUpsample(256 + 64, 256)
            self.predict = nn.Sequential(
                ...
                nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(256, self.output_channel,
                          kernel_size=1, stride=1, padding=0)
            )
...
if __name__ == '__main__':
    ...
    net = AAA(backbone='se_resnext50_32x4d').cuda()
