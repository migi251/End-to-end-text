...
class Resize(object):
    def __init__(self, maxHeight=512, maxWidth=512, dynamic=False):
    ...
    def __call__(self, image, polygons=None):
        h, w, _ = image.shape
        if self.dynamic and h*w < self.maxHeight*self.maxWidth:
            h = h - h % 32
            w = w - w % 32
...
class BaseTransform(object):
    def __init__(self, maxHeight, maxWidth, mean, std,dynamic =False):
        ...
        self.std = std
        self.augmentation = Compose([
            Resize(maxHeight=maxHeight, maxWidth=maxWidth, dynamic=False),
            Normalize(mean, std)
        ])
...
def build_transforms(maxHeight=1, maxWidth=1, batch_size=1, is_train=True,dynamic = False, **kwargs):
