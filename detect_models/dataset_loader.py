...
class DetectDataset(Dataset):
    ...
    def get_training_data(self, image, polygons, image_path):
        ...
        cnt =[]
        for p in polygons:
            cnt.append(p.points.astype(np.int32))
        cv2.drawContours(tmp_img,cnt,-1,(255,0,0),2)
        cv2.imwrite('./trash/'+im_name+'img.jpg', cv2.cvtColor(tmp_img,cv2.COLOR_RGB2BGR))
        if self.transform_image:
            image, polygons = self.transform_image(image, copy.copy(polygons))
        # imagenet_mean = [0.485, 0.456, 0.406]
        # imagenet_std = [0.229, 0.224, 0.225]
        # cv2.imwrite('./trash/'+im_name+'img_tranpose.jpg',
        ...
        #         if weight[i][j] != 0:
        #             img[i][j] = 255
        # cv2.imwrite('./trash/'+im_name+'vec_word.jpg',
        #                         img.astype(np.uint8))
        # print(im_name)
